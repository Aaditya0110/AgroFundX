...
public LoanList setUsersLoansList(int userid, int loanid, getLoanDto loanListDto) {
    ...
    if (loanList != null && app_User != null && loans != null) {
        loanList.setAppUser(app_User);
        // System.out.println(loanListDto.getAmt());
        loanList.setAmt(loanListDto.getAmt()); 
        loanList.setStatus("Pending");
        loanList.setLoans(loans);
        loanList.setBalance(loanListDto.getBalance()); 
...
public List<UsersLoans> getUsersLoans(int userid){
        ...
        for(LoanList temp:loanList){
            ...
            userloan.setBalance(temp.getBalance());
